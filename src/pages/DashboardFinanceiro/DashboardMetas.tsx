...
const numero = (value: string | number) =>
    ...
    <Stack direction="row" sx={{ minHeight: "100vh" }}>
      ...
      >
        ...
        <Stack direction={{ xs: "column", sm: "row" }} gap={2}>
          {[
            ["Metas não arquivadas", ativas.length],
            [
              "Metas atingidas",
              ativas.filter((meta) => meta.status === "atingida").length,
            ],
          ].map(([label, value]) => (
            <Paper
              key={label}
        ...
        >
          <Stack
            ...
            mb={2}
          >
            <Typography variant="h5" sx={{ fontFamily: fonts.hero }}>
              Minhas metas
            </Typography>
          ...
          {loading ? (
            ...
            <Stack gap={2}>
              ...
              {metas
                .filter((meta) => arquivadas || !meta.arquivada)
                .map((meta) => (
                  <Stack
                    key={meta.id}
                    sx={{
                  ...
                  >
                    ...
                    <Typography variant="body2">
                      {numero(meta.progresso)}% do avanço planejado · Prazo:{" "}
        ...
        <Alert severity="info">
          As análises automáticas e os indicadores financeiros ainda não estão
          integrados. Valores demonstrativos foram retirados deste painel.
        </Alert>
        <Button
          sx={{ alignSelf: "flex-start" }}
          onClick={() => navigate("/dashboard-redes")}
        >
          Consultar minhas redes
        </Button>
        <Dialog
          open={open}
          onClose={() => {
